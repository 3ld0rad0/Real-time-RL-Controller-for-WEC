...
export default function Results({
  ...
  const handleDeleteRun = async (runId: string) => {
    setIsDeleting(true)
    try {
      const success = await window.api.deleteResultRun(runId)
      if (success) {
        const updated = await window.api.getResults()
        setResults(updated)
        handleBack()
      } else {
        alert("Failed to delete result files.")
      }
    } catch (err) {
      console.error(err)
    ...
    <div className="p-10 w-full flex flex-col gap-6 h-[calc(100vh-2rem)] animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="px-3.5 py-2 text-xs font-bold rounded-xl border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700 transition-all flex items-center gap-2 cursor-pointer shadow-sm ml-2"
          >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full border border-slate-100 shadow-2xl flex flex-col gap-6 animate-scale-up">
            <div>
              <h3 className="text-xl font-bold text-slate-900 mb-1">Delete Simulation Run?</h3>
              <p className="text-sm text-slate-400">
                Are you sure you want to permanently delete this simulation run? This will delete all associated CSV data files and generated plot images. This action cannot be undone.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                disabled={isDeleting}
                onClick={() => handleDeleteRun(expandedFile.id)}
              ...
              <button
                ...
                className="w-full py-3.5 px-6 bg-slate-100 hover:bg-slate-200 text-slate-655 font-semibold rounded-xl transition-colors cursor-pointer"
              >
                Cancel
              </button>
            </div>
